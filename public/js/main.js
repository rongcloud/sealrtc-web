(function (dependencies) {
  ...
  function getSelfMediaStream(videoEnable, audioEnable, resolution) {
    var videoConfig = videoEnable || false;
    ...
    return new Promise(function (resolve, reject) {
      ...
      //   resolve(user);
      // }, reject);
      rongRTCStream.get(constraints).then(function(stream) {
        var user = {
          id: loginUserId,
          stream: {
            mediaStream: stream.mediaStream,
  ...
  function addCustomVideoBox(user, videoId) {
    ...
    videoDom.loop = true;
    if (isSelf) {
      videoDom.src = './mediaresource/video_demo' + videoId + '.mp4';
      var count = 0;
  ...
  function publishSelfMediaStream(videoEnable, audioEnable, resolution, frameRate, audioOnly) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      getSelfMediaStream(videoEnable, audioEnable, resolution, audioOnly,frameRate).then(function (user) {
        console.timeEnd('get ')
        addUserStream(user); //获取到本地后直接渲染到页面，防止己端发布成功后才渲染
        if (!videoEnable) {
          closeVideo(user);
        }
        rongRTCStream.publish(user).then(function () {
  ...
  function switchLocalVideo(e) {
    ...
    addCustomVideoBox({ id: loginUserId }, currentTargetVideo.replace('rong-customVideo', ''));
  }
  ...
  function bindRTCBtnEvent() {
    ...
    Dom.getById('rong-customVideo3').onclick = switchLocalVideo;
  ...
  function RTCNumberCheck(peopleNum, params) {
    ...
    if (params.bystanderEnable == false) {
      if (peopleNum < LimitNum.SRJoinNumAV) {
        ...
        publishSelfMediaStream(
          (videoEnable && videoDeviceId ? { deviceId: { exact: videoDeviceId } } : videoEnable),
          audioEnable && audioDeviceId ? { deviceId: { exact: audioDeviceId } } : audioEnable,
          resolution,
          frameRate,
        ).then(utils.noop, publishStreamError);
  ...
  * @param {boolean} params.audioDeviceId 选中的音频输入设备 ID
  * @param {boolean} params.videoDeviceId 选中的视频输入设备 ID
  */
  var startRTC = function (params) {
    ...
    joinRoom(params.roomId, params.token).then(function (roomUsers) {
      ...
      common.UI.showUserId(loginUserId);
